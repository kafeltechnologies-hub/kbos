<div class="min-h-screen bg-slate-100 p-6 text-slate-900">
    ...
    @include('livewire.finance._header', [
        ...
        'subtitle' => 'Generate, review, print and export financial, project, tax and management reports.'
    ])
    ...
    <div class="bg-white border border-slate-300 shadow-sm overflow-hidden mb-6">
        ...
        <div class="bg-slate-900 text-white px-4 py-3">
            <p class="text-[10px] uppercase tracking-widest text-green-300 font-bold">Company / Tax Information</p>
            <h2 class="text-sm font-black">Print Header Details</h2>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white border border-slate-300 shadow-sm overflow-hidden">
            <div class="bg-slate-900 text-white px-4 py-3">
                <p class="text-[10px] uppercase tracking-widest text-blue-300 font-bold">Financial Summary</p>
                <h2 class="text-sm font-black">Debit And Credit Position</h2>
            </div>

            <div class="divide-y divide-slate-200">
                <div class="p-4 flex justify-between">
                    <span class="text-xs font-bold">Total Debit</span>
                    <span class="font-mono font-black text-green-700">{{ number_format((float) ($summary['total_debit'] ?? 0), 2) }}</span>
                </div>

                <div class="p-4 flex justify-between">
                    <span class="text-xs font-bold">Total Credit</span>
                    <span class="font-mono font-black text-red-700">{{ number_format((float) ($summary['total_credit'] ?? 0), 2) }}</span>
                </div>

                <div class="p-4 bg-slate-900 text-white flex justify-between font-black">
                    <span>Difference</span>
                    <span class="font-mono">
                        {{ number_format((float) (($summary['total_debit'] ?? 0) - ($summary['total_credit'] ?? 0)), 2) }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 shadow-sm overflow-hidden">
            <div class="bg-slate-900 text-white px-4 py-3">
                <p class="text-[10px] uppercase tracking-widest text-green-300 font-bold">Cash Summary</p>
                <h2 class="text-sm font-black">Receipts And Payments</h2>
            </div>

            <div class="divide-y divide-slate-200">
                <div class="p-4 flex justify-between">
                    <span class="text-xs font-bold">Total Receipts</span>
                    <span class="font-mono font-black text-green-700">{{ number_format((float) ($summary['total_receipts'] ?? 0), 2) }}</span>
                </div>

                <div class="p-4 flex justify-between">
                    <span class="text-xs font-bold">Total Payments</span>
                    <span class="font-mono font-black text-red-700">{{ number_format((float) ($summary['total_payments'] ?? 0), 2) }}</span>
                </div>
        ...
        <div class="bg-white border border-slate-300 shadow-sm overflow-hidden">
            <div class="bg-slate-900 text-white px-4 py-3">
                ...
                <h2 class="text-sm font-black">Statutory Exposure</h2>
            </div>

            <div class="divide-y divide-slate-200">
                <div class="p-4 flex justify-between">
                    <span class="text-xs font-bold">Tax Captured</span>
                    <span class="font-mono font-black text-amber-700">{{ number_format((float) ($summary['total_tax'] ?? 0), 2) }}</span>
                </div>

                <div class="p-4 flex justify-between">
                    <span class="text-xs font-bold">Report Type</span>
                    <span class="text-xs font-black">{{ str_replace('_', ' ', strtoupper($reportType ?? 'NONE')) }}</span>
                </div>

                <div class="p-4 bg-amber-50">
                    <p class="text-xs font-black text-amber-800">
                        Generate tax-ready reports with company TIN, VAT number and report period.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white border border-slate-300 shadow-sm overflow-hidden mb-6">
        ...
        <div class="bg-slate-800 text-white px-4 py-3">
            <p class="text-[10px] uppercase tracking-widest text-green-300 font-bold">Print Centre</p>
            <h2 class="text-sm font-black">Report Printing, PDF And Excel Export</h2>
        </div>
    ...
    <div class="bg-white border border-slate-300 shadow-sm overflow-hidden mb-6">
        ...
        <div class="bg-slate-900 text-white px-4 py-3">
            <p class="text-[10px] uppercase tracking-widest text-green-300 font-bold">Accounting Reports</p>
            <h2 class="text-sm font-black">{{ $reportTitle ?? 'Generated Report' }}</h2>
        </div>
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    @forelse($reportRows ?? [] as $row)
                        ...
                        @php
                            ...
                            $source = $isArray
                                ? ($row['source'] ?? 'report')
                                : ($row->source ?? $row->module ?? $row->source_module ?? 'report');
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white border border-slate-300 shadow-sm overflow-hidden">
            ...
            <div class="bg-slate-900 text-white px-4 py-3">
                <p class="text-[10px] uppercase tracking-widest text-amber-300 font-bold">Tax Exposure</p>
    ...
    <div class="bg-white border border-slate-300 shadow-sm overflow-hidden">
        ...
        <div class="bg-slate-900 text-white px-4 py-3">
            <p class="text-[10px] uppercase tracking-widest text-green-300 font-bold">Management Notes</p>
            <h2 class="text-sm font-black">Prepared, Reviewed And Approved For Reporting</h2>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-5">

            <div class="border border-slate-300 bg-slate-50 p-4">
                <p class="text-[10px] uppercase font-bold text-slate-500">Prepared By</p>
                <p class="text-sm font-black">{{ auth()->user()->name ?? 'System User' }}</p>
            </div>

            <div class="border border-slate-300 bg-slate-50 p-4">
                <p class="text-[10px] uppercase font-bold text-slate-500">Reviewed By</p>
                <p class="text-sm font-black">Finance Manager</p>
            </div>

            <div class="border border-slate-300 bg-slate-50 p-4">
                <p class="text-[10px] uppercase font-bold text-slate-500">Approved By</p>
                <p class="text-sm font-black">Managing Director</p>
            </div>
